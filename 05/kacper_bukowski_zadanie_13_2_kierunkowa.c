...
void print_students(){
    ...
    printf("%12s | %12s | %5s | %30s | %10s | %16s | %16s | %16s \n", "IMIE", "NAZWISKO", "ROK", "ADRES", "STYPENDIUM", "ADRES OBIEKTU", "ADRES PREV", "ADRES NEXT");
    while (current != NULL){
...
void print_student(STUDENT *student) {
    printf("%12s | %12s | %5d | %30s | %10.2lf | %16p | %16p | %16p\n", student->imie, student->nazwisko, student->rok, student->adres, student->stypendium, student, student->prev, student->next);
}
...
void delete_first_student(){
    head = head->next;
    free_student(head->prev);
    head->prev = NULL;
}

void delete_last_student(){
    tail = tail->prev;
    free_student(tail->next);
    tail->next = NULL;
}